...
public class Tester {
	public static void readInput(String inputFile, ArrayList<Course> courses, ArrayList<Student> students)
			throws InvalidMarkException, FileNotFoundException, NoSuchElementException {
	...
	public static double[] parseMarks(String line, String studentID, double[] maxMarks, String courseName)
			throws InvalidMarkException {
		String[] markStringArray = line.trim().split("\\s+");

		if (markStringArray.length != maxMarks.length) {
			throw new InvalidMarkException(
					"Invalid number of marks for student " + studentID + " in course " + courseName +
		...
		for (int i = 0; i < markStringArray.length; i++) {
			try {
				...
				if (marks[i] > maxMarks[i]) {
					throw new InvalidMarkException("Invalid mark for student " + studentID + ": " + marks[i]
							+ "\nThe mark exceeds the maximum mark of " + maxMarks[i] + " for assessment " + (i + 1)
